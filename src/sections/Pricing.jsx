        ...
        <div className='container'>
          ...
          <div className="scroll-hide relative z-2 -mt-12 flex items-start max-xl:gap-5 max-xl:overflow-auto max-xl:pt-6">
            {plans.map((plan, index) => (
              // Cápsula de los planes (los impares [1] tienen lg:odd:mt-12 -> elemento central esta elevado respecto de los otros)
              <div
                key={plan.id}
                className="pricing-plan_first pricing-plan_last pricing-plan_odd pricing-plan_even relative border-2 p-7 max-xl:min-w-80 max-lg:rounded-3xl xl:w-[calc(33.33%+2px)]"
              >
                {/* Gradiente del plan [1]*/}
                {index === 1 && (
                  <div className="g4 absolute h-330 left-0 right-0 top-0 z-1 rounded-tl-3xl rounded-tr-3xl" />
                )}

                {/* Imagen de encabezado del plan */}
                <div
                  className={clsx(
                    "absolute left-0 right-0 z-2 flex items-center justify-center",
                    index === 1 ? "-top-6" : "-top-6 xl:-top-11", // En el plan [1] está elevado un poco más 
                  )}
                >
                  <img
                    src={plan.logo}
